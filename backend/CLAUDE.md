...
DeerFlow is a LangGraph-based AI super agent system with a full-stack architecture. The backend provides a "super agent" with sandbox execution, runtime long-term memory, subagent delegation, and extensible tool integration - all operating in per-thread isolated environments.

**Architecture**:
- **LangGraph Server** (port 2024): Agent runtime and workflow execution
- **Gateway API** (port 8001): REST API for models, MCP, skills, artifacts, uploads, and local thread cleanup
...
│   │   │   └── routers/       # FastAPI route modules (models, mcp, skills, uploads, threads, artifacts, agents, suggestions, channels)
...
- System prompt generated by `apply_prompt_template()` with skills and subagent instructions
...
**Components**:
- `mem0_client.py` - Thin Mem0 OSS Python SDK wrapper used by the runtime
- `updater.py` - Debounced persistence path; for `provider=mem0` it writes filtered conversation messages by `user_id`
- `queue.py` - Debounced update queue carrying both `thread_id` and `user_id`
- `prompt.py` - Request-time memory formatting helpers for injection middleware

**Workflow**:
1. BFF forwards authenticated `user_id` in the stream context
2. `MemoryMiddleware.abefore_model()` retrieves relevant memories from Mem0 and injects them as a request-time reminder
3. `MemoryMiddleware.after_agent()` filters user/final-AI messages and queues them with `thread_id` + `user_id`
4. Queue debounces (30s default), batches updates, and deduplicates per-thread
5. `MemoryUpdater` persists the conversation to Mem0 via `add_messages(...)`
...
- `storage_path` - File-path fallback for `provider=file`
- `debounce_seconds` - Wait time before processing (default: 30)
- `mem0_search_limit` - Retrieval count for request-time injection
- `write_enabled` - Master switch for post-run memory writes
- `mem0_config` - Config passed to `Memory.from_config(...)`
